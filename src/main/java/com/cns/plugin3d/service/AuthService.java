...
public class AuthService {
    ...
    public LoginResponse login(LoginRequest loginRequest) {
        ...
        try {
            ...
            String refreshToken = jwtUtil.generateToken(user.getEmail(), user.getRole().name(), REFRESH_TOKEN_EXP);
    ...
    public LoginResponse refreshToken(RefreshRequest request) {
        ...
        return new LoginResponse(newAccess, token, ACCESS_TOKEN_EXP,
                new UserDTO(user.getId(), user.getRole().name(), user.getEmail()));
    }
    ...
    public LoginResponse register(RegisterRequest request) {
        ...
        String refreshToken = jwtUtil.generateToken(newUser.getEmail(), newUser.getRole().name(), 7*24*3600);

        refreshTokenRepository.save(new RefreshToken(newUser, refreshToken));
